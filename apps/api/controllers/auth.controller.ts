...
import { generateSecret, generateURI, verify } from '../utils/two-fa.util'
...
export const postReset = async (req: Request, res: Response) => {
  ...
  return NodeMailModel.sendResetPasswordEmail(
    email,
    NodeMailModel.getResetPasswordTokenLink(email, token)
  )
    .then(() => res.redirect('/'))
    .catch((err: any) => {
      throw new Error(err)
...
export const putProfile = async (req: Request, res: Response, next: NextFunction) => {
  ...
  try {
    ...
    return profile.save().then((updatedProfile) => res.status(200).json(updatedProfile))
  } catch (error: Error | any) {
    return next(error)
  }
}
