...
export class ProfileComponent implements OnInit, OnDestroy {
  ...
  openTwoFAModal(): void {
    ...
    this.profileService
      .get2fa()
      .pipe(take(1), shareReplay(), distinctUntilChanged())
      .subscribe({
        next: (response) => {
  ...
  private save(): void {
    ...
    this.profileUpdateSub$ = this.profileService
      .updateProfile(formValue)
      .pipe(distinctUntilChanged(), shareReplay(), debounceTime(500))
      .subscribe((value) => {
  ...
  private getErrorMessage(error: HttpErrorResponse, fallback: string): string {
    const body = error.error as
      { message?: string; error?: string | string[] } | string | undefined;
